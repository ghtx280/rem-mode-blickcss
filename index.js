...
function RemMode(params = {}) {
    return (blick) => {
        ...
        const toRem = obj => params => {
            obj._values = params.parseValue(obj)?.map(e => (
                +e.raw ? e.raw / (grade || 16) + "rem" : e.raw
            ))
            return obj
        }
        ...
        blick.class.text = (e) => {
            ...
            text_obj._prop = (
                +e.value?.[0] || text_obj._vals[e.value] ? "font-size: $" : "color: $"
            )
        ...
        blick.attr.grid._else = 
        blick.attr.flex._else = 
        e => +e.style[0] ? toRem({ _prop: 'gap:$' }) : undefined
        
        blick.attr.text._else = e => (
            +e.style[0] ? toRem({ _prop: 'font-size:$' }) : { _prop: 'color:$' }
        )
